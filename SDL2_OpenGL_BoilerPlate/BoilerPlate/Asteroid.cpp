...
Asteroid::Asteroid() {
	...
	//big size default
	position = new Vector2(static_cast<float>(rand()), static_cast<float>(rand()));
	asteroidSize = 3;
...
void Asteroid::AsteroidMovement() {
	...
	entityVelocity->x = (asteroidMovementSpeed / mass) * -sinf(mathUtilities.degreesToRadians(static_cast<float>(asteroidOrientation))) + asteroidSize;
	entityVelocity->y = (asteroidMovementSpeed / mass) * cosf(mathUtilities.degreesToRadians(static_cast<float>(asteroidOrientation))) + asteroidSize;
}
